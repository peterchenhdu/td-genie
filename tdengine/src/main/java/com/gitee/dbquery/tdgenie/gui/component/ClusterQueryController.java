 ...
 * @since 2024/01/31
 **/
@ViewController(value = "/fxml/component/cluster_tab.fxml")
public class ClusterQueryController {
    ...
    @FXML
    private JFXComboBox<String> nodeTypeComboBox;
    @FXML
    ...
    public void init() {
        ...
        nodeTypeComboBox.getItems().add("数据节点");
        nodeTypeComboBox.getItems().add("管理节点");
        nodeTypeComboBox.getSelectionModel().select(0);
    ...
    private void query(Map<String, Object> queryMap) {
        if (connectionComboBox.getSelectionModel().isEmpty() || nodeTypeComboBox.getSelectionModel().isEmpty() ) {
            return;
        }

        String connectionName = connectionComboBox.getSelectionModel().getSelectedItem();
        String nodeType = nodeTypeComboBox.getSelectionModel().getSelectedItem();
        ...
        String sql = nodeType.equals("数据节点") ? "show dnodes;" : "show mnodes;";
